...
export function logBotStatus(result) {
  console.log(`[bot-status] ${JSON.stringify(result)}`);
}
...
export function parseBotJoinLine(line) {
  const payload = parseTaggedJson(line, '[bot-join]');
  if (!payload || !JOIN_STATUSES.includes(payload.status)) return null;
  return payload;
}

export function parseBotStatusLine(line) {
  const payload = parseTaggedJson(line, '[bot-status]');
  if (!payload || typeof payload !== 'object') return null;
  return payload;
}

export function parseBotResultLine(line) {
  const payload = parseTaggedJson(line, '[bot-result]');
  if (!payload || typeof payload !== 'object') return null;
  return payload;
}

export function parseChatSentLine(line) {
  const text = String(line || '');
  const match = text.match(/Messages sent:\s*(\d+)/i);
  if (!match) return null;
  return { sent: Number.parseInt(match[1], 10) };
}
