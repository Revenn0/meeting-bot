...
export const MAX_FLEET_SIZE = 15;

export function clampFleetSize(size) {
  const n = Number.parseInt(String(size ?? ''), 10);
  if (!Number.isFinite(n) || n < 1) return 1;
  return Math.min(n, MAX_FLEET_SIZE);
}

export function assertFleetSize(size) {
  const n = Number(size);
  if (!Number.isFinite(n) || n < 1) {
    throw new Error('FLEET_SIZE must be a positive integer.');
  }
  if (n > MAX_FLEET_SIZE) {
    throw new Error(
      `FLEET_SIZE cannot exceed ${MAX_FLEET_SIZE} real Meet guests (got ${n}).`,
    );
  }
  return n;
}
